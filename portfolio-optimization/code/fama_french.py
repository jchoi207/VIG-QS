...
class fama_french():
    def __init__(self, path: str,  period: str = '1y', num_securities: int = 1, risk_free_rate: int = 0.0382, display_metrics: bool = False):
        ...
        self.get_beta_SMB_HML(display_metrics=display_metrics)
        self.regression()
    ...
    def initialize_attribs(self):
        ...
        self.end_ = self.today_year - 0  # change this int to the window time frame
        ...
        for year in range(self.start_, self.end_):
            ...
            for ticker in self.tickers:
                try:
                    ...
                    se_yearly[ticker] = metrics1.get_bs_from_ticker(ticker=ticker,
                                                                   start=start,
                                                                   end=end,
                                                                   table=False,
                                                                   plot=False,
    ...
    def regression(self):
        ...
        for ticker in self.tickers:
            try:
                num_backtests = 1
                train_index = self.period_int - num_backtests  # 1 testing size
                ...
                for test in range(num_backtests):
                    prediction = beta_1 * \
                        x_1_test[test] + beta_2 * x_2_test[test] + \
                        epsilon + c_test[test] + self.risk_free_rate
                    ground_truth = r_test[test]
...
f = fama_french(path = tickers_path, period='5y',num_securities=2, display_metrics=False)